...
import { getMyPurchasedBatches, getBatchDetails } from '../services/api';
import BatchExplorer from './BatchExplorer';

const PurchasedBatchesSection = () => {
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      ...
      if (token) {
        const data = await getMyPurchasedBatches(token);
        setBatches(data || []);
      }
  ...
  const handleEnroll = (e: React.MouseEvent, batch: any) => {
    ...
    if (!isAlreadyEnrolled) {
      ...
      alert(`Successfully enrolled in ${batch.name || 'Batch'}!`);
    } else {
    ...
    <div className="p-4 sm:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-body-2 font-medium">Batches you have purchased</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBatches.map((batch, index) => {
            ...
            return (
              <div 
                key={batch._id || batch.id || batch.slug || index}
        ...
        <div className="bg-white rounded-3xl border border-stroke-light p-20 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-headings mb-2">No purchased batches found</h3>
          <p className="text-sm text-body-2 font-medium">You have not purchased any batches yet.</p>
        </div>
      )}
    </div>
  );
...
export default PurchasedBatchesSection;
